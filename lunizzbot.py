...
class Bot(commands.Bot):
    ...
    async def on_message(self, message):
        ...
        if message.author.bot == client.user:
            return
        try:
            #                modz                    lunizzers            vips                  interfazes              droids
            priviliged = ["605125919643926617", "605126315166924830", "605141526636396565", "701051279388049438", "605133230135312386"]
            priv_channel = ["605130756729077762"]
            channels = []
            keywords = []
            id = ["<@!181008524590055424>","<@181008524590055424>"]
            for i in message.author.roles:                  
                if str(i.id) in priviliged:
                    return   
                            ...
                            for i in data:
                                for y in r_y_msg:                                                   
                                    if y in data[i]["keywords"]:   # çıktının hangi kanal ve kannalara ait olacağını belirlemek için keywordsleri bir listeye atıyoruz.
                                        ...
                                        continue
                        if len(channels) == 0:
                            if ( id[0] in  message.content ):
                                await message.channel.send("> {} \nDostum bu etiketin işe yarayacağına gerçekten emin misin? <@{}>".format(message.content.replace(id[0],"Lunizz"),message.author.id))
                            elif(id[1] in message.content ):
                                await message.channel.send("> {} \nDostum bu etiketin işe yarayacağına gerçekten emin misin? <@{}>".format(message.content.replace(id[1],"Lunizz"),message.author.id))
                            else:
                                return
                        elif len(channels) == 1:
                            if (id[0] in  message.content ):
